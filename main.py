...
class Game:
    def __init__(self, *, enemies, player):
        ...
        self.isVisited['entrance_hall'] = False
        self.isVisited['armory'] = False
    ...
    def play(self):
        self.Audio_Source.set_Sound('Rayos.wav')
        ...
        Text_Service.typing_effect("De repente, una voz grave te saca de tus pensamientos:\n")
        Text_Service.Dialogue('¿Vas a entrar o dejarás que el miedo te consuma?', name = "Voz misteriosa")
        Text_Service.typing_effect("\nTe giras rápidamente, buscando la fuente de la voz, pero no ves a nadie. Solo la oscuridad del castillo te da la bienvenida.")
        
        Text_Service.typing_effect("Te decides y empujas las puertas con esfuerzo. Crujen ruidosamente al abrirse, revelando el vestíbulo del castillo.")
    ...
    def entrance_hall(self):
        ...
        if not self.isVisited[self.location]:
            ...
            self.Audio_Source.set_Sound('closeDoor.wav')
            self.Audio_Source.Start_async(sleep=0.3, position=(1,0,1))
            
            Audio_Source2 = Sound_Service()
            Audio_Source2.set_Sound('closeDoor.wav')
            Audio_Source2.Start_async(sleep=0.4, position=(-1,0,-1))
            
            Audio_Source3 = Sound_Service()
            Audio_Source3.set_Sound('closeDoor.wav')
            Audio_Source3.Start_async(sleep=0.5, position=(-1,0,1))
            
            Audio_Source4 = Sound_Service()
            Audio_Source4.set_Sound('closeDoor.wav')
            Audio_Source4.Start_async(sleep=0.7, position=(1,0,-1))
            ...
            self.Audio_Source.set_Sound('pasos.wav')
            self.Audio_Source.Start_async(duration=25, looping=True)
        ...
        if action == 'norte':
            self.change_Room('throne_room')
        elif action == 'este':
            self.change_Room('library')
        elif action == 'oeste':
            self.change_Room('armory')
        elif action == 'sur':
            self.change_Room('dungeon')
    ...
    def armory(self):
        ...
        if 'sword' not in self.player.inventory:
            ...
            if action == 'hablar':
                ...
                self.Audio_Source.set_Sound('tesoro2.wav')
                self.Audio_Source.Start_async()
        ...
        if action == 'examinar':
            ...
            self.change_Room('entrance_hall')
        else:
            print("No entiendes qué hacer.")
    ...
    def library(self):
        ...
        if not self.isVisited[self.location]:
            ...
            self.Audio_Source.set_Sound('birds.wav')
            self.Audio_Source.Start_async(gain=0.5, sleep= 14)

            Audio_Source2 = Sound_Service()
            Audio_Source2.set_Sound('bookPage.wav')
            Audio_Source2.Start_async(sleep=0.4, position=(-1,0,-1), gain=0.4, looping= True, duration=6)
            
            Audio_Source3 = Sound_Service()
            Audio_Source3.set_Sound('bookPage.wav')
            Audio_Source3.Start_async(sleep=0.5, position=(-1,0,1), gain=0.4, looping= True, duration=6)
            
            Audio_Source4 = Sound_Service()
            Audio_Source4.set_Sound('bookPage.wav')
            Audio_Source4.Start_async(sleep=0.7, position=(1,0,-1), gain=0.4, looping= True, duration=6)

            
            Text_Service.typing_effect("A medida que avanzas, notas que algunos libros flotan suavemente en el aire, moviéndose entre los estantes como si fueran aves, sus páginas susurrando secretos olvidados.")
        ...
        if action == 'examinar':
            self.Audio_Source.set_Sound('tesoro1.wav')
            self.Audio_Source.Start_async(gain=0.8)
            ...
            if action == 'continuar':
                self.change_Room('secret_chamber')
        elif action == 'volver':
            self.change_Room('entrance_hall')
        else:
            print("No entiendes qué hacer.")
    ...
    def dungeon(self):
        ...
        if not self.isVisited[self.location]:
            self.isVisited[self.location] = True
            self.Audio_Source.set_Sound('vientoFuerte.wav')
            self.Audio_Source.Start_async(gain=0.4)
        ...
        if action == 'explorar':
            Text_Service.typing_effect("Te acercas a las celdas, examinando cada una con detenimiento. En una de ellas, encuentras un esqueleto encorvado en un rincón oscuro.")
    ...
    def throne_room(self):
        ...
        if not self.isVisited[self.location]:
            ...
            self.Audio_Source.set_Sound('tesoro2.wav')
            self.Audio_Source.Start_async(gain=0.7, position=(-1,0,0))
        ...
        if action == 'examinar':
            ...
            self.Audio_Source.set_Sound('heartBit.wav')
            self.Audio_Source.Start_async()
    ...
    def secret_chamber(self):
        ...
        if not self.isVisited[self.location]:
            self.isVisited[self.location] = True
            self.Audio_Source.set_Sound('vientoFuerte.wav')
            self.Audio_Source.Start_async(gain=0.5)
        ...
        if action == 'insertar':
            if 'sword' in self.player.inventory: 
                if all(enemy.isDefeated for enemy in enemies.values()):
                    self.Audio_Source.set_Sound('tesoro3.wav')
                    self.Audio_Source.Start_async(gain=0.5)
                    ...
                    Audio_Source2 = Sound_Service()
                    Audio_Source2.set_Sound('coin.wav')
                    Audio_Source2.Start_async(sleep=0.4, position=(-1,0,-1))
                    
                    Audio_Source3 = Sound_Service()
                    Audio_Source3.set_Sound('coin.wav')
                    Audio_Source3.Start_async(sleep=0.5, position=(-1,0,1))
                    
                    Audio_Source4 = Sound_Service()
                    Audio_Source4.set_Sound('coin.wav')
                    Audio_Source4.Start_async(sleep=0.7, position=(1,0,-1))            
                    ...
                    Text_Service.typing_effect(", insertas la espada en el hueco del pedestal... Pero no para nada")
            else:
    ...
    def enemy_encounter(self, room):
        if not self.enemies[room] == None: pass
        if not self.enemies[room].isDefeated:
            ...
            if self.enemies[room].isDiscovered == False:
                if 'lantern' in self.player.inventory:
                    ...
                    if use == 'si':
                        self.Audio_Source.set_Sound('Linterna.wav') 
                        self.Audio_Source.Start()
                        self.Audio_Source.set_Sound(enemies[room].sound)
                        x = 1 if random.randint(0, 10) % 2 == 0 else -1
                        self.Audio_Source.Start_async(position=(x,0,0))
    ...
    def end_game(self):
        ...
        print(f"Jugaste por {int(elapsed_time)} segundos.")
...
enemies = {
            ...
            'library': Enemy('espectro de la biblioteca', 'espectroBiblioteca.wav'),
            'dungeon': Enemy('carcelero de la mazmorra', 'carcelero.wav'),
            'throne_room': Enemy('rey espectral', 'reyEspectral.wav'),
            'secret_chamber': Enemy('guardián de la reliquia', 'guardianReliquia.wav')
        }
